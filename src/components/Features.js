...
export const fetchCalc = async (columns, method, startDate, endDate) => {
    return axios
        .post(`${process.env.REACT_APP_API_BASE_URL}api/calc/`, {
            columns: columns, method: method.toLowerCase(), startDate: startDate, endDate: endDate
...
const CSVColumns = ({
                        columns, setData, handleOpen, method, selectedColumns, setSelectedColumns,
                    }) => {
    ...
    return (<>
        <Form onFinish={onFinish}>
            ...
            >
                {columns.map((item) => (<Checkbox
                    style={{
                        gap: '6px', fontSize: '20px',
                    }}
                    onChange={onChange}
...
const Features = ({
                      setData, handleOpen, method, columns, setColumns, setSelectedColumns, selectedColumns,
                  }) => {
    useEffect(() => {
        const fetch = async () => {
    ...
    return (<Card
        ...
        style={{
            width: '100%', marginTop: '60px', marginBottom: '60px',
        }}
    >
        <CSVColumns
            columns={columns}
